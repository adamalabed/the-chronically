    ...
    if (activeArticleId) {
      setArticles(articles.map(a => 
        a.id === activeArticleId 
          ? { ...a, title: editorTitle, categories: editorCategories, content: editorContent }
          : a
      ));
    } else {
      const newArticle = {
        ...
        {view === 'admin' && isAuthenticated && (
          <div className="mt-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className={`flex justify-between items-center mb-12 border-b pb-6 ${isDark ? 'border-gray-800' : 'border-gray-200'}`}>
              <h2 className="text-2xl sm:text-3xl font-bold" style={{ fontFamily: "'Playfair Display', serif" }}>{UI_TEXT.editorsDesk}</h2>
