...
const UI_TEXT = {
  ...
  empty: "No entries yet. Unlock the restricted area to publish."
};
...
export default function App() {
  // --- STATE ---
  const [articles, setArticles] = useState([]); // Starts empty for database fetch
  ...
  const deleteArticle = async (id) => {
    if (window.confirm(UI_TEXT.deletePrompt)) {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${isDark ? 'dark bg-[#121212] text-gray-100' : 'bg-[#FAFAFA] text-gray-900'}`} style={{ fontFamily: "'Lora', serif" }}>
      ...
      <header className="max-w-4xl mx-auto px-6 pt-16 pb-12 flex flex-col items-center relative">
        {/* Dark Mode & Admin Controls (Top Right) */}
      ...
      <main className="max-w-3xl mx-auto px-6 pb-24 min-h-[60vh]">
        ...
        {view === 'home' && (
          <div className="space-y-16 sm:space-y-20 mt-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            {isLoading ? (
              <p className="text-center italic opacity-50 mt-20">{UI_TEXT.loading}</p>
            ) : articles.length === 0 ? (
              <p className="text-center italic opacity-50 mt-20">{UI_TEXT.empty}</p>
            ) : (
        ...
        {view === 'article' && activeArticle && (
          <article className="mt-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            >
              <ArrowLeft size={16} className="mr-2" /> {UI_TEXT.back}
            </button>
            ...
            <header className="mb-10 sm:mb-14 text-center">
              <p className={`text-xs sm:text-sm uppercase tracking-widest mb-4 sm:mb-6 font-sans font-medium ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                {UI_TEXT.publishedOn} {activeArticle.date}{activeArticle.categories && activeArticle.categories.length > 0 ? ` / ${activeArticle.categories.join(', ')}` : ''}
              </p>
              <h1 
        ...
        {view === 'admin' && isAuthenticated && (
          <div className="mt-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className={`flex justify-between items-center mb-12 border-b pb-6 ${isDark ? 'border-gray-800' : 'border-gray-200'}`}>
              <h2 className="text-2xl sm:text-3xl font-bold" style={{ fontFamily: "'Amarante', serif" }}>{UI_TEXT.editorsDesk}</h2>
              <div className="flex space-x-2 sm:space-x-4">
                ...
                >
                  <Plus size={16} className="mr-1 sm:mr-2" /> {UI_TEXT.newEntry}
                </button>
                <button 
            ...
            <div className="space-y-4">
              {articles.map(article => (
                <div key={article.id} className={`p-4 sm:p-6 rounded-lg flex justify-between items-center border transition-colors ${isDark ? 'border-gray-800 bg-[#1A1A1A] hover:border-gray-700' : 'border-gray-200 bg-white hover:border-gray-300'}`}>
                  <div className="pr-4">
                    <h3 className="text-lg sm:text-xl font-bold mb-1 sm:mb-2 line-clamp-1" style={{ fontFamily: "'Amarante', serif" }}>{article.title}</h3>
        ...
        {view === 'editor' && isAuthenticated && (
          <div className="mt-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            >
              <ArrowLeft size={16} className="mr-2" /> {UI_TEXT.cancel}
            </button>

            <input
              type="text"
              placeholder={UI_TEXT.headline}
              value={editorTitle}
            ...
            <div className={`mb-8 p-4 sm:p-5 rounded border text-xs sm:text-sm font-sans ${isDark ? 'bg-[#1A1A1A] border-gray-800 text-gray-300' : 'bg-gray-50 border-gray-200 text-gray-600'}`}>
              <strong>{UI_TEXT.fmtGuide}</strong> {UI_TEXT.fmtDesc}
            </div>

            <textarea
              placeholder={UI_TEXT.writeHere}
              value={editorContent}
      ...
      {showLoginModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-300">
          <div className={`w-full max-w-sm p-8 rounded shadow-2xl animate-in zoom-in-95 ${isDark ? 'bg-[#121212] border border-gray-800' : 'bg-white'}`}>
            <h3 className="text-2xl sm:text-3xl font-bold mb-8 text-center" style={{ fontFamily: "'Amarante', serif" }}>
              {UI_TEXT.restricted}
            </h3>
            <form onSubmit={handleLogin}>
              <input 
                type="password" 
                placeholder={UI_TEXT.passcode}
                autoFocus
                value={passwordInput}
              ...
              />
              {loginError && <p className="text-red-500 text-sm mb-6 font-sans text-center">{UI_TEXT.incorrect}</p>}
